            ...
            samfile_t *fp_out)
{
  ...
  if(NULL != args->P) {
      ...
      if(j < tmp || 0 != strncmp(args->P, name, tmp)) {
          dwgsim_eval_print_error(FnName, name, "[dwgsim_eval] could not match read name with given read name prefix (-P)", Warn, OutOfRange);
          free(name);
          return;
      }
      i = tmp + 1; 
  ...
  if(14 != sscanf(name+i, "%s %d %d %1d %1d %1d %1d %d %d %d %d %d %d %s",
		  chr_name, &pos_1, &pos_2, &str_1, &str_2, &rand_1, &rand2,
                  ...
                  read_num)) {
      dwgsim_eval_print_error(FnName, name+i, "[dwgsim_eval] read was not generated by dwgsim?", Warn, OutOfRange);
      free(name);
      return;
  }
  free(name);
